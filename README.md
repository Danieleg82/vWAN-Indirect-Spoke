...
-	If you are **not** using _vWAN BGP endpoints_ (https://learn.microsoft.com/en-us/azure/virtual-wan/scenario-bgp-peering-hub) you need to be able to summarize/aggregate the IP ranges of the spoke VNETs connected to your transit HUB as static routes in the Default Route table of the vWAN HUB. In such case, keep in mind that **Routing Intent** will not be available at all on your vHUB: in fact, **Routing Intent is incompatible with any kind of static route configured in the Default Route Table of a vHUB, which are not pointing to the local FW as nexthop** (https://learn.microsoft.com/en-us/azure/virtual-wan/how-to-routing-policies#knownlimitations )
